...
<body>
	...
	function computePizzaTotal($pizza, $toppings, $qty){
   	...
   	if (!empty($toppings)) {
        	foreach ($toppings as $topping) {
                	...
                	$toppingPrice += $toppingsMenu[$topping];
           		}
       		}
   	}

    $totalPerPizza = $basePrice + $toppingPrice;

    return $totalPerPizza * $qty;
}
    ?>
    ...
    <div class="container">
        ...
        <div class="content">
			...
			echo "<input type='radio' name = 'pizza' value = '$pizza' required>";
			echo "<label for ='pizza $pizza'> $pizza <span class = 'price'>P$price</span>";
			echo "</div>";
			}
			?>
			...
			<?php
			foreach ($toppingsMenu as $toppings => $price){
			echo "<div class = 'checkbox.item'>";
			echo "<input type='checkbox' name = 'toppings[]' value = '$toppings' required>";
			echo "<label for ='toppings $toppings'> $toppings <span class = 'price'>P$price</span>";
			echo "</div>";
			}
			?>
                            </div>
			...
			$total = computePizzaTotal($pizza, $toppings, $quantity);
			...
			$toppingsList = !empty($toppings) ? implode(", ", $toppings) : "None";
			echo "<div class='summary-item'><span>Toppings:</span><span>$toppingsList</span></div>";

			echo "<div class='total'>Total: P" . number_format($total, 2) . "</div>";
                    } else {
		...
		echo "<div class='menu-grid'>";
			...
			foreach ($pizzaMenu as $key => $value){
    			echo "<div class='menu-item'>";
    			echo "<strong>$key</strong><br>";
    			echo "<span class='price'>P$value</span>";
    			echo "</div>";
			}
		echo "</div>";
		?>
